    ...
    fn decompress_file(&mut self) {
        ...
        if let (Ok(file), Ok(target)) = (input, output) {
            ...
            if copy(&mut reader, &mut writer).is_ok() {
                ...
                self.status.push_str("\nNote: After decompression, remember to change the file extension to its original type for a seamless experience.");
                self.progress = 1.0;
            } else {
...
impl epi::App for MyApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &epi::Frame) {
        ...
        egui::CentralPanel::default().frame(frame).show(ctx, |ui| {
            ...
            ui.vertical_centered(|ui| {
                ui.add_space(20.0);
                ui.heading(egui::RichText::new("Quantum File Compressor Decompressor")
    ...
    fn name(&self) -> &str {
        "Quantum File Compressor - By Marmik Mewada"
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // let icon_path = "C:\\Users\\admin\\Desktop\\rs\\compress_youtube\\src\\icon.ico";
    let icon_path = r"C:\Users\admin\Desktop\rs\compress_youtube\src\icon.ico";
    println!("Icon path is: {}", icon_path);
    // Load the icon data
    let icon_data = std::fs::read(icon_path)?;
    println!("Icon data length: {}", icon_data.len());

    let icon_data = epi::IconData {
        rgba: icon_data,
        width: 32, // Use the actual width from your icon
        height: 32, // Use the actual height from your icon
    };
...
// fn main() -> Result<(), Box<dyn std::error::Error>> {
//     let app = MyApp::default();
//     let icon_path = "C:\\Users\\admin\\Desktop\\rs\\compress_youtube\\src\\icon.ico"; // Adjust the path accordingly
